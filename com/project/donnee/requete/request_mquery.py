...
tableau = {'actinoid': actinoid, 'alkali': alkali, 'alkaline': alkaline, 'chalcogen': chalcogen, 'halogen': halogen,
           'lanthanoid': lanthanoid, 'metalloid': metalloid, 'noble_gas': noble_gas, 'quadrupolar': quadrupolar,
           'rare_earth_metal': rare_earth_metal, 'transition_metal': transition_metal}
...
def request(group):
    results = api.query(
        {'elements': {'$in': tableau.get(group)}, 'nelements': {'$lte': 6}, 'elasticity': {'$ne': None}},
        properties=['pretty_formula', 'elasticity'])
    N = len(results)
