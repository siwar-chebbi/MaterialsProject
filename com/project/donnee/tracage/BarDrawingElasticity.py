...
def drawTable(tableauSource, propsTableauToPlot, pdffile):
    ...
    for prop in propsTableauToPlot:
        ...
        for i in range(0, col):
            abscisses.append(
                i + 1 + propsTableauToPlot.index(prop) * epaisseur - int(len(propsTableauToPlot) * epaisseur / 2))

        maxY = max(maxY, (tableauSource[propsTableau.index(prop), :]).max())
        plt.bar(abscisses, tableauSource[propsTableau.index(prop), :], color=couleur, width=epaisseur, label=prop[11:],
                align='center')  # bar est le defaut
    # maxY = 3
